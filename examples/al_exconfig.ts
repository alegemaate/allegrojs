...
function main() {
  ...
  if (data.length != 3) {
    ...
    allegro_message(
      `Found ${data.length} parameters in graphics.mode instead of the 3 expected.\n`
    );
    w = 320;
    h = 200;
    bpp = 8;
  ...
  if (data.length != 3) {
    /* We expect only 3 parameters */
    allegro_message(
      `Found ${data.length} parameters in content.headercolor instead of the 3 expected.\n`
    );
    r = g = b = 255;
  } else {
    r = atoi(data[0]);
  ...
  if (set_gfx_mode("canvas_id", windowed, w, h, 0, 0) != 0) {
    allegro_message(
      "Unable to set mode " + w + "x" + h + " with " + bpp + "bpp\n"
    );
    free(filename);
    free(title);
    exit(-1);
