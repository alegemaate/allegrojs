...
var canvas: AllegroCanvas | null = null;
...
function set_gfx_mode(
  ...
  var cv = document.getElementById(canvas_id) as HTMLCanvasElement;
  if (!cv) {
...
function _strokestyle(bitmap: AllegroBitmap, colour: number, width: number) {
  if (!width) width = 1;
...
/// @return colour in 0xAARRGGBB format
function makecolf(r: number, g: number, b: number, a: number) {
...
/// @param y y coordinate of pixel
/// @param colour colour in 0xAARRGGBB format
function putpixel(bitmap: AllegroBitmap, x: number, y: number, colour: number) {
...
function clear_bitmap(bitmap: AllegroBitmap) {
...
/// @param bitmap bitmap to be cleared
/// @param colour colour in 0xAARRGGBB format
function clear_to_color(bitmap: AllegroBitmap, colour: number) {
  bitmap.context.clearRect(0, 0, bitmap.w, bitmap.h);
  _fillstyle(bitmap, colour);
...
/// @return font object
function load_base64_font(data: string) {
...
/// @return font object
function load_font(filename: string) {
  var s = document.createElement("style");
  var fontname = "font" + _num_fonts++;
  s.id = fontname;
  s.type = "text/css";
...
var _debug_element: HTMLElement | null = null;
...
function enable_debug(id: string) {
